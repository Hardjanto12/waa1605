...
<table id="dataTable" class="table table-striped"  width="894" border="0" cellspacing="1" cellpadding="2">
  <tr>
    ...
    <td width="78" bgcolor="#CCCCCC"><strong>Proyek</strong></td>
    <td width="73" bgcolor="#CCCCCC"><strong>Mesin </strong></td>
    <td width="62" bgcolor="#CCCCCC"><strong>Tanggal </strong></td>
    <td width="64" bgcolor="#CCCCCC"><strong>No Transaksi </strong></td>
    <td width="78" bgcolor="#CCCCCC"><strong>Remark</strong></td>
    <td width="91" bgcolor="#CCCCCC"><strong>Acc/Inv</strong></td>
    <td width="63" bgcolor="#CCCCCC"><strong>Nama </strong></td>
	...
	$mySql = "select f.* from (select a.date, a.prj, a.msn, convert(varchar,a.date,111) as tgl, a.nojnl, a.remark, a.inv, a.namainv, a.qin, a.qout, a.vin, a.vout from acp a where left(a.nojnl,3) not in ('PBL','PJL') union all select a.date, a.prj, a.msn, convert(varchar,a.date,111) as tgl, a.nojnl, a.remark, a.acc as inv, b.nmac as namainv, 0 as qin, 0 as qout, 0 as vin, a.debet as vout from acd a inner join acc b on a.acc=b.kdac) f WHERE f.date>='$tanggal_11' and f.date<='$tanggal_22' ";
//		echo trim($txtInv);
//		echo $mySql . " and b.inv='$txtInv' ";
		if (trim($txtPrj)<>""){
			$mySql = $mySql . " and f.prj='$txtPrj' ";
		}
		if (trim($txtMsn)<>""){
			$mySql = $mySql . " and f.msn='$txtMsn' ";
		}
...
//		echo $mySql;
		...
		while ($myData = sqlsrv_fetch_array($myQry)) {
			$nomor++;
			$tnil= $tnil + ($myData['vout']);
			$tjml= $tjml + ($myData['qin']);
		?>
		<tr>
		...
		<tr>
			...
			<td>&nbsp</td>
			<td>TOTAL :</td>
			<td><?php echo format_angka($tjml); ?></td>
			<td>&nbsp</td>
			<td><?php echo format_angka($tnil); ?></td>
		</tr>
		<?php 
		?>
</table>
...
$mulai	= 0; //$baris * ($hal-1); 
?>
<h2>LAPORAN HPP PRODUKSI PER PERIODE </h2>
