...
//		echo $noJurnal;
 		$mySql	= "update pbd set stt='1' where nojnl='$txtKodeNas' ";
		$stmt = sqlsrv_query( $koneksidb, $mySql);
		if( $stmt === false ) {
			 die( print_r( sqlsrv_errors(), true));
		}
...
//	$txtNamaInv			= $_POST['txtNamaInv'];
   ...
   # Validasi Form Input 
	...
	if (count($pesanError)>=1 ){
		...
		$mySql	= "INSERT INTO pbd (nojnl, acc, prj, msn, inv, ref, remark, qty, grup, dtseq,stt) VALUES ('$Kode','$txtTab','$txtPrj','$txtMsn','$txtInv','',(select name from inv where inv='$txtInv'),'$txtQty','1',(select isnull(MAX(dtseq),0) from pbd where nojnl='$txtKodeNas')+1,'2')";
		$stmt = sqlsrv_query( $koneksidb, $mySql);
		if( $stmt === false ) {
			 die( print_r( sqlsrv_errors(), true));
		}
		...
		echo "<meta http-equiv='refresh' content='0; url=?open=Pemakaian-Edit&Kode=$Kode&Tanda=2'>";
		exit;	
	}
}
...
$mySql 	= "SELECT a.*,  convert(varchar,a.date,111) as tgl FROM pbh a  where a.nojnl='$Kode'  ";
//echo $mySql;
$myQry 	= sqlsrv_query($koneksidb,$mySql) ;
$nom  = 0; 
$myData= sqlsrv_fetch_array($myQry);
...
//echo $Tanda;
if ($Tanda==1){
		$mySql	= "delete from pbd where nojnl='$Kode' and stt='2' ";				
		$stmt = sqlsrv_query($koneksidb,$mySql);	
		if( $stmt === false ) {
			die( print_r( sqlsrv_errors(), true));
		}
$mySql 	= "SELECT a.* FROM pbd a inner join inv b on a.inv=b.inv where a.nojnl='$Kode' and a.stt='1' ORDER BY a.nojnl, a.dtseq ASC ";
} else {
$mySql 	= "SELECT a.* FROM pbd a inner join inv b on a.inv=b.inv where a.nojnl='$Kode'  ORDER BY a.nojnl, a.dtseq ASC ";
}
//echo $mySql;
$myQry 	= sqlsrv_query($koneksidb,$mySql) ;
$nomor  = 0; 
