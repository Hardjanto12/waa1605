...
if($dataTahun and $dataBulan) {
	$filterSQL = "WHERE a.control='7' and substring(a.nojnl,6,2)='$Tahun' AND substring(nojnl,8,2)='$dataBulan'"; 
}
else {
	$filterSQL = "WHERE a.control='7'"; 
}

# UNTUK PAGING (PEMBAGIAN HALAMAN)
$baris	= 50;
$hal 	= isset($_GET['hal']) ? $_GET['hal'] : 1;
$infoSql= "SELECT * FROM pbh $filterSQL"; 
...
<title>Approval Pemakaian Bahan</title></head>

<body>
<h1> APPROVAL PEMAKAIAN BAHAN </h1>
...
<form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" name="form1" target="_self">
  ...
  <tr>
    ...
    <td colspan="2" align="right"><a href="?open=Pemakaian-Add" target="_self"><img src="images/btn_add_data.png" width="140" height="34" border="0"></a></td>
  </tr>
  </table>
</form>
...
<table id="dataTable" class="table-list" width="946" border="0" cellspacing="2" cellpadding="3">
  <tr>
    ...
    <td width="120" bgcolor="#CCCCCC"><strong>No. Jurnal </strong></td>
    <td width="149" bgcolor="#CCCCCC"><strong>Remark</strong></td>
    <td width="124" bgcolor="#CCCCCC"><div align="left"><strong>Created</strong></div></td>
    <td width="73" bgcolor="#CCCCCC"><div align="left"><strong>User </strong></div></td>
...
$mySql 	= "SELECT a.*,convert(varchar,a.date,111) as tgl, case when a.control='1' then 'SAVED' when a.control='7' then 'WAITING' else 'DELETED' end as stts FROM pbh a  $filterSQL ORDER BY a.date ASC ";
//echo $mySql;
...
?>
  <tr>
    ...
    <td> <?php echo $myData['stts']; ?>  </td>
        <td width="24" align="center"><a href="?open=Pemakaian-Approve&Kode=<?php echo $Kode; ?>&Tanda=<?php echo "1"; ?>" target="_self" 
			onclick="return confirm('YAKIN INGIN APRROVE DATA BAGIAN INI ... ?')">Approve</a></td>
        <td width="39" align="center"><a href="?open=Pemakaian-Reject&Kode=<?php echo $Kode; ?>" target="_self" 
			onclick="return confirm('YAKIN INGIN REJECT DATA BAGIAN INI ... ?')">Reject</a></td>
    <td width="41"><a href="?open=Pemakaian-Cetak&Kode=<?php echo $Kode; ?>" target="_self">View</a></td>
  </tr>
...
<?php } ?>
  ...
  <tr>
	...
	<?php
	for ($h = 1; $h <= $maks; $h++) {
		echo " <a href='?open=Penjualan-Data&hal=$h'>$h</a> ";
	}
	?> </strong> </td>
  </tr>
</table>
