...
//	$txtNamaInv			= $_POST['txtNamaInv'];
   ...
   # Validasi Form Input 
	...
	$pesanError = array();
	if (trim($txtSub)=="") {
		$pesanError[] = "Data <b>Kode Customer</b> tidak boleh kosong !";		
	}
...
//		echo $noJurnal;
 		$mySql	= "update kld set stt='1' where nojnl='$txtKodeNas' ";
		$stmt = sqlsrv_query( $koneksidb, $mySql);
		if( $stmt === false ) {
			 die( print_r( sqlsrv_errors(), true));
		}
...
//	$txtNamaInv			= $_POST['txtNamaInv'];
   ...
   # Validasi Form Input 
	...
	if (count($pesanError)>=1 ){
		...
		$mySql	= "INSERT INTO kld (nojnl, acc, inv, ref, remark, qty, price, val, disc, Vdisc, grup, dtseq,stt) VALUES ('$Kode','$txtTab','$txtInv','',(select name from inv where inv='$txtInv'),'$txtQty','$txtPrice',$txtQty*($txtPrice-$txtDisc),$txtDisc,0,'1',(select isnull(MAX(dtseq),0) from kld where nojnl='$txtKodeNas')+1,'2')";
		echo $mySql;
		$stmt = sqlsrv_query( $koneksidb, $mySql);
		if( $stmt === false ) {
			 die( print_r( sqlsrv_errors(), true));
		...
		echo "<meta http-equiv='refresh' content='0; url=?open=Penjualan-Edit&Kode=$Kode&Tanda=2'>";
		exit;	
	}
}
...
$mySql 	= "SELECT a.*, b.name, convert(varchar,a.date,111) as tgl, case when a.ppn='1' then 'NON PPN' when a.ppn='2' then 'INCLUDE' else 'EXCLUDE' end as sppn FROM klr a left outer join sub b on a.sub=b.sub  where a.nojnl='$Kode'  ";
//echo $mySql;
$myQry 	= sqlsrv_query($koneksidb,$mySql) ;
$nom  = 0; 
$myData= sqlsrv_fetch_array($myQry);
...
//echo $Tanda;
if ($Tanda==1){
		$mySql	= "delete from kld where nojnl='$Kode' and stt='2' ";				
		$stmt = sqlsrv_query($koneksidb,$mySql);	
		if( $stmt === false ) {
			die( print_r( sqlsrv_errors(), true));
		}
$mySql 	= "SELECT a.* FROM kld a inner join inv b on a.inv=b.inv where a.nojnl='$Kode' and a.stt='1' ORDER BY a.nojnl, a.dtseq ASC ";
} else {
$mySql 	= "SELECT a.* FROM kld a inner join inv b on a.inv=b.inv where a.nojnl='$Kode'  ORDER BY a.nojnl, a.dtseq ASC ";
}
//echo $mySql;
$myQry 	= sqlsrv_query($koneksidb,$mySql) ;
$nomor  = 0; 
...
?>
    <tr>
	  ...
	  <td width="56" align="center"><a href="?open=Penjualan-Delete-Detail&Kode=<?php echo $Kode; ?>&Seq=<?php echo $Seq; ?>" target="_self">Delete</a></td>
    </tr>
<?php } ?>
    <tr>
      <td>&nbsp;</td>
