...
//	$txtNamaInv			= $_POST['txtNamaInv'];
   ...
   # Validasi Form Input 
	...
	if (count($pesanError)>=1 ){
		...
		$noJurnal 	= buatKodeJurnal("msk", "PBL","nojnl",9,$koneksidb,trim($txtTanggal));
//		echo $noJurnal;
...
//		echo $noJurnal;
 		$mySql	= "update msd set nojnl='$noJurnal' where nojnl='$txtKodeNas' ";
		$stmt = sqlsrv_query( $koneksidb, $mySql);
		if( $stmt === false ) {
			 die( print_r( sqlsrv_errors(), true));
		}
 		$mySql	= "delete from msd where nojnl='$txtKodeNas' ";
		$stmt = sqlsrv_query( $koneksidb, $mySql);
		if( $stmt === false ) {
			 die( print_r( sqlsrv_errors(), true));
		}
...
if(isset($_POST['btnKosong'])){
	...
	// Skrip Validasi form
		...
		$my2Sql	= "delete from msk where nojnl='$txtKodeNas' ";				
		$stmt2 = sqlsrv_query($koneksidb,$my2Sql);	
		if( $stmt2 === false ) {
			die( print_r( sqlsrv_errors(), true));
		}
		echo "<meta http-equiv='refresh' content='0; url=?open=Pembelian-Add'>";
		exit;	
...
//	$txtNamaInv			= $_POST['txtNamaInv'];
   ...
   # Validasi Form Input 
	...
	$mySql	= "select count(*) as jml from msk where nojnl='" . trim($txtKodeNas) . "' ";
	$myQry = sqlsrv_query( $koneksidb, $mySql);
	$myData= sqlsrv_fetch_array($myQry);
	if($myData['jml']==0) {
		$mySql	= "INSERT INTO msk (nojnl, date, acc, sub, remark, val, disc, vdisc, ppn, grup, chtime, chuser,control,nprint) VALUES ('$txtKodeNas','$txtTanggal','301','$txtSub','$txtKeterangan',0,0,0,'$jppn','1',getdate(),'$userLogin','6',1)";
		echo $mySql+"1";
		$stmt = sqlsrv_query( $koneksidb, $mySql);
		if( $stmt === false ) {
			 die( print_r( sqlsrv_errors(), true));
		...
		$mySql	= "delete from msk where nojnl='". trim($txtKodeNas) ."'";
		$stmt = sqlsrv_query( $koneksidb, $mySql);
		if( $stmt === false ) {
			 die( print_r( sqlsrv_errors(), true));
		}
		...
		$mySql	= "INSERT INTO msd (nojnl, acc, inv, ref, remark, qty, price, val, disc, Vdisc, grup, dtseq,stt) VALUES ('$txtKodeNas','$txtTab','$txtInv','',(select name from inv where inv='$txtInv'),'$txtQty','$txtPrice',$txtQty*($txtPrice-$txtDisc),$txtDisc,0,'1',(select isnull(MAX(dtseq),0) from msd where nojnl='$txtKodeNas')+1,'1')";
		echo $mySql;
		$stmt = sqlsrv_query( $koneksidb, $mySql);
		if( $stmt === false ) {
			 die( print_r( sqlsrv_errors(), true));
...
$mySql 	= "SELECT a.*, b.name, convert(varchar,a.date,111) as tgl, case when a.ppn='1' then 'NON PPN' when a.ppn='2' then 'INCLUDE' else 'EXCLUDE' end as sppn FROM msk a left outer join sub b on a.sub=b.sub  where a.nojnl='$userLogin'  ";
//echo $mySql;
$myQry 	= sqlsrv_query($koneksidb,$mySql) ;
$nom  = 0; 
$myData= sqlsrv_fetch_array($myQry);
...
<?php
// Skrip menampilkan data Transaksi Pinjaman
$mySql 	= "SELECT a.* FROM msd a inner join inv b on a.inv=b.inv where a.nojnl='$userLogin' ORDER BY a.nojnl, a.dtseq ASC ";
//echo $mySql;
$myQry 	= sqlsrv_query($koneksidb,$mySql) ;
$nomor  = 0; 
$ttl = 0;
