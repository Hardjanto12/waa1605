...
<table id="dataTable" class="table table-striped"  width="894" border="0" cellspacing="1" cellpadding="2">
  <tr>
    ...
    <td width="91" bgcolor="#CCCCCC"><strong>Gudang</strong></td>
    ...
    <td width="54" bgcolor="#CCCCCC"><strong>Qty In</strong></td>
    <td width="54" bgcolor="#CCCCCC"><strong>Qty Out</strong></td>
    <td width="54" bgcolor="#CCCCCC"><strong>Qty Saldo</strong></td>
    <td width="54" bgcolor="#CCCCCC"><strong>Nilai In </strong></td>
    <td width="54" bgcolor="#CCCCCC"><strong>Nilai Out </strong></td>
    <td width="54" bgcolor="#CCCCCC"><strong>Nilai Saldo </strong></td>
  </tr>
	    <?php 
		// Skrip menampilkan data Peminjaman dengan filter Periode
	$mySql = "select f.* from (select '$tanggal_11' as date,'SALDO AWAL' as nojnl,'' as remark,'' as sub,'' as namasub,a.acc,a.inv,a.namainv,sum(a.qin-a.qout) as qin,0 as qout,sum(a.vin-a.vout) as vin, 0 as vout,'' as prj,'' as msn,convert(varchar,max(a.date)-1,111) as tgl from acp a where a.date<'$tanggal_11' group by a.acc,a.inv,a.namainv union all select a.date,a.nojnl,a.remark,a.sub,a.namasub,a.acc,a.inv,a.namainv,a.qin,a.qout,a.vin,a.vout,a.prj,a.msn,convert(varchar,a.date,111) as tgl from acp a) f  WHERE f.date>='$tanggal_11' and f.date<='$tanggal_22' ";
//		echo trim($txtInv);
//		echo $mySql . " and b.inv='$txtInv' ";
		if (trim($txtTab)<>""){
			$mySql = $mySql . " and f.acc='$txtTab' ";
		}
		if (trim($txtInv)<>""){
			$mySql = $mySql . " and f.inv='$txtInv' ";
		}
			$mySql = $mySql . " order by f.acc, f.inv, f.date ";
...
//		echo $mySql;
		...
		while ($myData = sqlsrv_fetch_array($myQry)) {
			$nomor++;
			if ($nomor==1){
				$sblm == $myData['acc'] . $myData['inv'];
			}
			$ttl= $ttl + $myData['vin']-$myData['vout'];
			$tqty= $tqty + $myData['qin']-$myData['qout'];
			if ($sblm == $myData['acc'] . $myData['inv']) {
			} else {
				$sblm = $myData['acc'] . $myData['inv'];
				?>
				</tr>
...
//			echo $sblm;
		...
		<tr>
			...
			<td><?php echo format_angka($myData['qin']); ?></td>
			<td><?php echo format_angka($myData['qout']); ?></td>
			<td><?php echo format_angka($tqty); ?></td>
			<td><?php echo format_angka($myData['vin']); ?></td>
			<td><?php echo format_angka($myData['vout']); ?></td>
...
$mulai	= 0; //$baris * ($hal-1); 
?>
<h2>LAPORAN STOCK PER PERIODE </h2>
