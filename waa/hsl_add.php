...
//	$txtNamaInv			= $_POST['txtNamaInv'];
   ...
   # Validasi Form Input 
	...
	if (count($pesanError)>=1 ){
		...
		$noJurnal 	= buatKodeJurnal("hsl", "PRD","nojnl",9,$koneksidb,trim($txtTanggal));
//		echo $noJurnal;
...
//		echo $noJurnal;
 		$mySql	= "update hsd set nojnl='$noJurnal' where nojnl='$txtKodeNas' ";
		$stmt = sqlsrv_query( $koneksidb, $mySql);
		if( $stmt === false ) {
			 die( print_r( sqlsrv_errors(), true));
		}
 		$mySql	= "delete from hsd where nojnl='$txtKodeNas' ";
		$stmt = sqlsrv_query( $koneksidb, $mySql);
		if( $stmt === false ) {
			 die( print_r( sqlsrv_errors(), true));
		}
...
if(isset($_POST['btnKosong'])){
	...
	// Skrip Validasi form
		...
		$my2Sql	= "delete from hsl where nojnl='$txtKodeNas' ";				
		$stmt2 = sqlsrv_query($koneksidb,$my2Sql);	
		if( $stmt2 === false ) {
			die( print_r( sqlsrv_errors(), true));
		}
		echo "<meta http-equiv='refresh' content='0; url=?open=Produksi-Add'>";
		exit;	
...
//	$txtNamaInv			= $_POST['txtNamaInv'];
   ...
   # Validasi Form Input 
	...
	$mySql	= "select count(*) as jml from hsl where nojnl='" . trim($txtKodeNas) . "' ";
	$myQry = sqlsrv_query( $koneksidb, $mySql);
	$myData= sqlsrv_fetch_array($myQry);
	if($myData['jml']==0) {
		$mySql	= "INSERT INTO hsl (nojnl, date, remark, grup, chtime, chuser,control,nprint) VALUES ('$txtKodeNas','$txtTanggal','$txtKeterangan','1',getdate(),'$userLogin','6',1)";
		$stmt = sqlsrv_query( $koneksidb, $mySql);
		if( $stmt === false ) {
			 die( print_r( sqlsrv_errors(), true));
		}
	} else {
		$mySql	= "delete from hsl where nojnl='". trim($txtKodeNas) ."'";
		$stmt = sqlsrv_query( $koneksidb, $mySql);
		if( $stmt === false ) {
			 die( print_r( sqlsrv_errors(), true));
		}
		...
		$mySql	= "INSERT INTO hsd (nojnl, acc, prj, msn, inv, ref, remark, qty, grup, dtseq,stt) VALUES ('$txtKodeNas','$txtTab','$txtPrj','$txtMsn','$txtInv','',(select name from inv where inv='$txtInv'),'$txtQty','1',(select isnull(MAX(dtseq),0) from hsd where nojnl='$txtKodeNas')+1,'1')";
		$stmt = sqlsrv_query( $koneksidb, $mySql);
		if( $stmt === false ) {
			 die( print_r( sqlsrv_errors(), true));
		}
...
<h1>Hasil Produksi Baru </h1>
  <table width="800" cellspacing="1"  class="table-list">

<?php
// Skrip menampilkan data Transaksi Pinjaman
$mySql 	= "SELECT a.*,  convert(varchar,a.date,111) as tgl FROM hsl a  where a.nojnl='$userLogin'  ";
//echo $mySql;
$myQry 	= sqlsrv_query($koneksidb,$mySql) ;
$nom  = 0; 
$myData= sqlsrv_fetch_array($myQry);
...
<?php
// Skrip menampilkan data Transaksi Pinjaman
$mySql 	= "SELECT a.*, b.name as namainv FROM hsd a inner join inv b on a.inv=b.inv where a.nojnl='$userLogin' ORDER BY a.nojnl, a.dtseq ASC ";
//echo $mySql;
$myQry 	= sqlsrv_query($koneksidb,$mySql) ;
$nomor  = 0; 
$jml  = 0; 
